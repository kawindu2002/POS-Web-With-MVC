...
$placeOrderBtn.on('click', function() {
    ...
    // Create a new order ID like "ORD001", "ORD002", etc.
    const newOrderId = 'ORD' + String(orders_db.length + 1).padStart(3, '0');
    // Find the customer object using the selected customer ID
    const selectedCustomer = customers_db.find(c => c.id === customerId);

    // Create a new order object with all required details
